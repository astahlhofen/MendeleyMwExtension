...
class BibEntry {
	...
	function BibEntry($type_specifier,$key_identifier, $cnt) {
		// Remove note-field from content. This can block the bibtext-popup.
		// TODO: This is not a good solution, because "note = { testnote {stupid thing}. going on }" gives ". going on�}" 
		$this->content = preg_replace("/note\s*?=\s*?\{.*?\}/", "", $cnt); 
	...
	function parse() {
	 ...
	 $counter = 0;
	 $publishing_details_order = array('institution', 'organization', 'city', 'author');
	 while ( $this->publishing_details == '' && $this->where_published == '' ) {
	 	$this->publishing_details .= $this->fields[ $publishing_details_order[$counter] ];
	 	$counter++;
	 	 
	 	if ( $counter == count( $publishing_details_order ) ) {
	 		break;
	 	}
	 }
